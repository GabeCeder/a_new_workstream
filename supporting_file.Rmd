...
hh %>% 
  filter(!is.infinite(number))
  mutate(number = case_when(number == "Inf" ~ "8",
                            number != "Inf" ~ number))
...
ggplot(data = mapping, mapping = aes(fill = per_100K_number,
                                                         geometry = geometry)) +
                # ,
...
hh
jj <- county_pop %>% 
  right_join(hh, by = c("state", "county"))
jj
saveRDS(jj, file = "analysis_app/data_files/test.rds")
jj
a
a <- jj %>% 
  filter(slice1 == "cases" &
                           slice2 == "cumulative") %>% 
                ggplot() +
                geom_sf(aes(fill = number, geometry = geometry))
hh
ggplotly(a)
```
