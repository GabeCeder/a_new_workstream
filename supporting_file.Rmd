...
vaccines <- read_csv(vaccines_text) %>% 
  ...
  filter(!(state %in% c("American Samoa",
                        ...
                        "United States"))) %>% 
  group_by(state) %>% 
  mutate(delta_doses_WoW = 100 * (0 - (1 - (seven_day_avg_doses_adm / 
...
c$number <- round(c$number, 0)
c$per_100K_number <- round(c$per_100K_number, 0)
...
saveRDS(c, file = paste("analysis_app/data_files/map_data", end_date, ".rds", sep = ""))
