...
vaccines2 <- read_csv(vaccines_text) %>% 
  ...
  select(date,
         "state" = long_name,
         "new_doses" = doses_distributed_daily,
        # "cumulative_doses_administered" = doses_administered,
         "today_seven_day_avg_doses_adm" = doses_administered_daily_7day_avg) %>% 
...
vaccines2 %>% filter(slice1 == "vax") %>% 
  ggplot() +
    geom_line(aes(x = date, y = today_seven_day_avg_doses_adm / 1000), color = "white") +
    geom_col(aes(x = date, y = new_doses / 1000), fill = "white", color = "white", alpha = 0.4) +
    # scale_x_date(date_labels = "%B", 
    #            date_breaks = "months", 
    #            name = "") +
    labs(x = "", y = "pasteDailyinput$select_view (thousands)",
         title = paste("In the past week, the United States averaged ", avg_daily_stat, " vaccinations per day.", sep = "")) +
    theme3
```
